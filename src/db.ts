...
export const DeliveryEvent = mongoose.model('DeliveryEvent', new Schema({ _id: String, projectId: String, sequence: Number, taskIds: [String], action: String, entityId: String, entityVersion: Number, at: Date }, { versionKey: false }));
...
export async function connect(uri: string) {
  ...
  await Promise.all([Project, Feature, Task, Execution, Event, TaskMessage, DeliveryEvent, AutomationPolicy, Runner, AutomationJob, MarkdownDocument, MarkdownRevision, Operation, Credential, Bootstrap].map(model => model.init()));
}
