...
export const Task = mongoose.model('Task', new Schema({ ...base, projectId: { type: String, index: true }, featureId: String, name: String,
  instructions: String, acceptance: [String], priority: Number, area: String, repositoryId: String, dependencies: [String],
...
export async function connect(uri: string) {
  ...
  await Promise.all([Project, Feature, Task, Execution, Event, TaskMessage, Operation, Credential, Bootstrap].map(model => model.init()));
}
